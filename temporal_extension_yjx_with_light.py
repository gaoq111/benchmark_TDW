...
def setup_scene(screen_size, render_quality, c: Controller, scene_name: str):
    """
    A simple function to add the scene and optional lighting,
    independent of your TemporalPositioning task.
    """
    ...
    commands.append(c.get_add_scene(scene_name))

    # Example: if you need special logic for certain scenes:
    if scene_name == "box_room_2018":
        interior_lighting = InteriorSceneLighting()
        interior_lighting.reset(
            hdri_skybox="snowy_forest_path_02_4k",
            aperture=9,
            focus_distance=2.5,
            ambient_occlusion_intensity=0.00,
            ambient_occlusion_thickness_modifier=1.5,
            shadow_strength=0.5
        )
        c.add_ons.append(interior_lighting)
        commands.append({"$type": "adjust_point_lights_intensity_by","intensity": 120.0})
        commands.append({"$type": "set_ambient_intensity","intensity":1.1})

    # Send the commands
    c.communicate(commands)
    print(f"Scene '{scene_name}' setup complete.")
...
class TemporalExtension(AbstractTask):
    def __init__(self,
                 controller: Controller,  # We pass in the existing controller
                 output_path: str = None,
                 scene: str = "empty_scene",
                 name: str = "temporal_position",
                 ...
                 **kwargs):
        """
        We store the existing controller. 
        We do NOT do 'Controller(...)' here because we want only one build.
        """
        # Skip calling super().__init__ with launch_build because we already have a controller
        self.c = controller
    ...
    def run(self, 
        ...
        try:
            ...
            #add third model as reference
            reference_objs = ['toy_monkey_medium', "h-shape_wood_block", "half_circle_wood_block", "heart", "hexagonal_toy"]
            self.commands.extend(self.c.get_add_physics_object(model_name=random.choice(reference_objs),
                                            library="models_core.json",
                                            gravity=False,
                                            position=get_bottom_position(scene),
            ...
            short_last = random.randint(0, MOVE_STEP - 3)
            diff  = random.randint(2, MOVE_STEP - 1 - short_last)
            long_last = short_last + diff

            start1 = random.randint(0, MOVE_STEP - 1 - short_last)
            end1 = start1 + short_last

            start2 = random.randint(0, MOVE_STEP - 1 - long_last)
            end2 = start2 + long_last
            ...
            for cam in self.camera:
                ...
                # ]
                output_res_cam[cam]["scene"] = scene
            ...
            self.c.communicate({"$type": "destroy_all_objects"})

@hydra.main(config_path="configs", config_name="temporal_extension.yaml", version_base=None)
def main(cfg: DictConfig):
    server_process = start_tdw_server(display=cfg.display, port=cfg.port)
    c = Controller(port=cfg.port, launch_build=False)
    ...
    SELECTED_SCENES = ["ruin","monkey_physics_room","box_room_2018","suburb_scene_2018"]
    scenes = SELECTED_SCENES
    #random.shuffle(scenes)
    for scene in SELECTED_SCENES:
        setup_scene(cfg.screen_size, cfg.render_quality, c,scene)
        for texture1, texture2 in texture_pairs[:2]:
            random.shuffle(size_pairs)
            for size1, size2 in size_pairs[:4]:
                random.shuffle(object_pairs)
                for object1, object2 in object_pairs[:8]: 
                    random.shuffle(color_pairs)
                    for color1, color2 in color_pairs[:10]:
                        random.shuffle(texture_pairs)
                        task = None
                        task = TemporalExtension(
        controller=c,
        output_path=cfg.output_path,
        name="temporal_extension"
    )
                        np.random.seed(seed)
                        seed += 1
                        main_obj = ObjectType(
                            model_name=object1,
                            library="models_core.json",
                            position=get_position_with_offset(scene, x_range, y_range, z_range, -0.4),
                            ...
                            motion=np.random.choice(["forward", "right"]),
                            color=color1
                        )
                        motion_map = {
                            "forward": "backward", 
                        ...
                        fixed_obj = ObjectType(
                            ...
                            object_id=None,
                            material="concrete_raw_damaged",
                            motion=motion_map[main_obj.motion],
                            color=color2
                        )
