...
def generate_color_shape_distribution(color_tuple, shape_tuple, combined_tuples):
    ...
    shape_chosen = random.choice(shape_tuple)

    for color_shape_tuple in combined_tuples:
        if color_shape_tuple[0] in colors_chosen and color_shape_tuple[1] == shape_chosen:
                ...
                for material in tqdm(object_materials, desc="Processing materials", leave=False):
                    for table, table_height in tqdm(tables.items(), desc="Processing tables", leave=False):
                        for question_type in ['near', 'far']:
                            ...
                            if question_type == "near":
                                x1 = random.uniform(-0.6, 0.6)
                                x2 = x1 + SIZE_SCALE * random.choice([-1, 1])
                                
                                z1 = random.uniform(-0.25, 0.25)
                                z2 = z1 + SIZE_SCALE * random.choice([-1, 1])

                            elif question_type == "far":
                                x1 = random.uniform(-0.6, 0.6)
                            ...
                            for (color_name, object_name), obj_num in tqdm(color_shape_dic.items(), desc="Processing objects", leave=False):
                                ...
                                if object_name == "prim_cube":
                                    object_full_name = "cube"
                                elif object_name == "prim_cyl":
                                    object_full_name = "cylinder"
                                elif object_name == "prim_sphere":
                                    object_full_name = "sphere"
                                output += color_name + " " + object_full_name + " " + str(obj_num) + "\n"
                            ...
                            for camera_position in tqdm(camera_positions, desc="Processing camera positions", leave=False):
                                ...
                                images_info["position_section"][-1]["question"] = f"Is a third {object_full_name} of the same size can be placed in between the two {object_full_name}s?"
                                if question_type == "near":
