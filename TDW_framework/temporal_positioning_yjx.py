...
from constants import AVAILABLE_OBJECT, ObjectType, AVAILABLE_MOTION, AVAILABLE_CAMERA_POS, AVAILABLE_COLOR, AVAILABLE_SCENE, AVAILABLE_SCALE_FACTOR
import hydra
import os
...
class TemporalPositioningTask(AbstractTask):
    ...
    def run(self, 
            main_obj_list: List[ObjectType],
            fixed_obj_list: List[ObjectType],
            seed: int = 12):
        ...
        main_object_names = [obj.model_name for obj in main_obj_list]
        fixed_object_names = [obj.model_name for obj in fixed_obj_list]
        ...
        try:
            move_object_dict: Dict[str, MoveObject] = {}
            ...
            self.cameras = get_camera_views(motion = main_obj.motion, camera_view=self.camera)
            self.c.communicate(self.commands)
            print("Complete cameras commands")
            ...
            for camera in self.cameras:
                self.c.add_ons.append(get_cameras(camera))
            print("cameras added: ", self.cameras)

            capture = ImageCapture(avatar_ids=self.cameras, path=os.path.join(self.output_path, self.name, self.expr_id), png=True)
            ...
            before_gen = len(os.listdir(os.path.join(self.output_path, self.name, self.expr_id, self.cameras[0])))
...
def main(cfg: DictConfig):
    ...
    seed = 0
    while len(gen_id_set) < 1:
        task = None
        task = TemporalPositioningTask(**cfg)
        np.random.seed(seed)
        seed += 1
        main_obj = ObjectType(
