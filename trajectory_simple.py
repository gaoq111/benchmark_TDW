...
def get_action_coordinates(action, start_point, end_point):
    if action is None:
        return None
    else:
        return generate_line_coords(num_points=30, start_point=start_point, end_point=end_point)
...
def main(args):
    ...
    try:
        ...
        # Define Trajectories
        trajactories = ['horizontal', 'vertical']
                ...
                for material in tqdm(object_materials, leave=False):
                    for traj in tqdm(trajactories, leave=False):
                        for _ in tqdm(range(num_data), leave=False):
                            ...
                            camera_config = config[scene]['camera']
                            object_config = config[scene]['object']
                            vision_boundary = config[scene]['vision_boundary']
                            ...
                            vertical = (traj=='vertical')
                            flip = random.choice([True, False])
                            if vertical:
                                x = random.uniform(vision_boundary['x'][0], vision_boundary['x'][1])
                                start_point = (x, vision_boundary['z'][1]) if flip else (x, vision_boundary['z'][0])
                                end_point = (x, vision_boundary['z'][0]) if flip else (x, vision_boundary['z'][1])
                            else:
                                z = random.uniform(vision_boundary['z'][0], vision_boundary['z'][1])
                                start_point = (vision_boundary['x'][1], z) if flip else (vision_boundary['x'][0], z)
                                end_point = (vision_boundary['x'][0], z) if flip else (vision_boundary['x'][1], z)

                            coordinates = get_action_coordinates(traj, start_point, end_point)
